...
class CLIP(nn.Module):
    ...
    def insert(self, args, tokenizer, device):
        ...
        if bool(getattr(args, "use_mapb", 0)) or self.score_mode == "prototype":
    ...
    def get_trainable_parameters(self):
        params = list([self.state_prompt_embedding]) + list(self.adaptor.parameters())
    ...
    def use_prototype_score(self, args):
        ...
        use_mapb = bool(getattr(args, "use_mapb", 0))
        return (use_mapb or score_mode == "prototype") and self.prototype_bank is not None
    ...
    def build_branch_average_map(self, branch_maps):
        return torch.stack(branch_maps, dim=1).mean(dim=1, keepdim=True)
    ...
    def detect_forward_seg(self, image, args, gts=None):
        ...
        for img_token in img_tokens:
            ...
            scores += score
            branch_prob = torch.softmax(score, dim=-1)
            branch_predict_map = branch_prob[:, 1:, 1]
        ...
        if self.use_prototype_score(args):
            ...
            if proto_aux is not None and float(proto_aux["prototype_fallback_ratio"].detach().item()) >= 1.0:
                ...
                proto_base_map = self.build_branch_average_map(proto_branch_maps)
